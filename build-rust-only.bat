...
REM =============================================================================
REM STEP 1: CREATE RUST CRATE WITH AUTOMATIC PROTOC
REM =============================================================================

echo.
echo 🦀 Creating Rust crate (Cargo will handle protoc automatically)...
...
REM =============================================================================
REM RUST BINDINGS WITH AUTOMATIC PROTOC
REM =============================================================================
...
REM =============================================================================
REM TEST RUST CRATE (OPTIONAL)
REM =============================================================================

echo.
echo 🧪 Testing Rust crate build (requires Rust toolchain^)...
...
if %ERRORLEVEL% equ 0 (
    ...
    if %ERRORLEVEL% equ 0 (
        ...
        if %ERRORLEVEL% equ 0 (
            ...
            echo   ⚠️  Some tests failed, but crate was built successfully
        )
    ) else (
        echo   ⚠️  Rust build failed, but crate files were created
        echo   💡 You can build it manually later with 'cargo build'
    )
    cd ..\..
) else (
    echo   ⚠️  Cargo not found - skipping build test
    echo   💡 Install Rust toolchain to test the build: https://rustup.rs/
)
